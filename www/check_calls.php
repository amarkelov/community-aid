...
/* POST method is not available for check 
...
else*/if(isset($_GET['clientid']) && !isset($_POST['submit'])) {
...
else*/if(isset($_GET['list']) && !isset($_POST['submit'])) {
...
/* I doubt that check has to alter the database!!!
 * 
...
if (isset($clean['submit']) and $clean['clientid']) {
	...
	$sql .= " VALUES ('{$clean['clientid']}', '{$mysql['chat']}', '{$clean['mclass']}') ";
	$result = mysql_query($sql, $dbConnect);
	if (!$result) {
		$message  = 'Invalid query: ' . mysql_error() . "\n";
...
if ($clean['clientid'] /* there's no submit available for check and !isset($clean['submit']) */) {
	$dbConnect = dbconnect();
...
<form method="post" action="<?php echo ($PHP_SELF . "?list=" . $clean['list']) ?>" >
